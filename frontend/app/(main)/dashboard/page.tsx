    ...
    async function loadData() {
      try {
        const promises: Promise<any>[] = [
          ...
          api.get('/dashboard/alerts').catch(() => ({ success: false, data: [] })),
          api.get('/dashboard/recent-applications').catch(() => ({ success: false, data: [] })),
        ];
        ...
        if (results[5]?.success) setAlerts(results[5].data || []);
        if (results[6]?.success) setRecentApps(results[6].data || []);
        if (results[7]?.success) setKpis(results[7].data);
      } catch (err) {
    ...
    <div className="animate-fade-in space-y-6">
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-2">
          ...
          <button className="hidden sm:inline-flex items-center gap-1.5 px-3 py-1.5 text-body-sm font-medium text-neutral-600 dark:text-neutral-300 border border-neutral-200 dark:border-neutral-700 rounded-lg hover:bg-neutral-50 dark:hover:bg-neutral-800 transition-colors">
            <Download size={14} /> Export
          </button>
          <Link href="/service-desk" className="hidden sm:inline-flex items-center gap-1.5 px-3 py-1.5 text-body-sm font-medium text-white bg-orange-500 hover:bg-orange-600 rounded-lg transition-colors">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        {loading ? (
          ...
          <>
            <KPICard title="Total Policies" value={summary?.totalPolicies || 0} icon={FileText} subtitle={`${summary?.activePolicies || 0} active`} variant="primary" animationDelay={0} trend={{ value: 12, isPositive: true }} />
            <KPICard title="Total Premium" value={formatCurrency(summary?.totalPremium || 0)} icon={DollarSign} variant="accent" animationDelay={75} trend={{ value: 8.5, isPositive: true }} />
            <KPICard title="Total Coverage" value={formatCurrency(summary?.totalCoverage || 0)} icon={TrendingUp} variant="secondary" animationDelay={150} trend={{ value: 15, isPositive: true }} />
            <KPICard title="Total Claims" value={claimsSummary?.totalClaims || 0} icon={AlertTriangle} subtitle={`${formatCurrency(claimsSummary?.totalClaimedAmount || 0)} total`} animationDelay={225} trend={{ value: 3.2, isPositive: false }} />
          </>
        )}
      </div>
          ...
          <div className="bg-white dark:bg-neutral-800 rounded-xl shadow-elevation-1 border border-surface-border dark:border-neutral-700 overflow-hidden">
            <div className="px-5 py-3.5 border-b border-surface-border dark:border-neutral-700 flex items-center justify-between">
              <div className="flex items-center gap-2">
                ...
                <h3 className="text-body-sm font-semibold text-neutral-700 dark:text-neutral-200">Recent Applications</h3>
              </div>
              <Link href="/service-desk" className="text-small font-medium text-accent-500 hover:text-accent-600 dark:text-accent-400 flex items-center gap-1">
                View All <ArrowRight size={12} />
              </Link>
            </div>
            <div className="divide-y divide-surface-border dark:divide-neutral-700">
              {recentApps.map((app) => (
                <div key={app.id} className="flex items-center justify-between px-4 py-3 hover:bg-neutral-50 dark:hover:bg-neutral-750 transition-colors cursor-pointer">
                  <div className="min-w-0">
                    <p className="text-body-sm font-medium text-accent-500">{app.id}</p>
                    <p className="text-small text-neutral-500 dark:text-neutral-400">{app.customer}</p>
                  </div>
                  <div className="text-right">
                    <p className="text-body-sm font-semibold text-neutral-900 dark:text-neutral-100">{formatCurrency(app.amount)}</p>
                    <div className="flex items-center gap-2 justify-end mt-0.5">
                      ...
                      <span className="text-[10px] text-neutral-400">{app.stage}</span>
                    </div>
                  </div>
                </div>
              ))}
              {recentApps.length === 0 && (
                <div className="px-4 py-8 text-center text-small text-neutral-400">No recent applications</div>
              )}
            </div>
          </div>
        </div>
